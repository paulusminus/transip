...
fn check_environment() -> Result<()> {
    ...
    if Path::new(&std::env::var(TRANSIP_API_PRIVATE_KEY).unwrap()).try_exists().is_err() {
        return Err(Error::EnvironmentVariable("Private key not found".to_owned()));
    }
    if !Path::new(&std::env::var(TRANSIP_API_LOG_DIR).unwrap()).exists() {
        return Err(Error::EnvironmentVariable("Log directory not found".to_owned()));
    }
    Ok(())
}
